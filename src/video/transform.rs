...
impl TransformConfig {
    ...
    pub fn for_resolution(input_height: Option<u32>) -> Self {
        ...
        if is_4k {
            // For 4K input: add 1080p as encoded stream and 2160p as original
            resolutions.insert(
                "1080p".to_string(),
                ResolutionConfig {
                    width: Some(1920),
            ...
            // For non-4K input: use original as highest quality
            resolutions.insert(
                "1080p".to_string(),
                ResolutionConfig {
                    is_original: true,
...
impl VideoProcessor {
    ...
    pub async fn transform(
        ...
        let result = self.collect_output_files(temp_dir).await?;
    ...
    async fn collect_output_files(&self, temp_dir: TempDir) -> Result<TransformResult, VideoError> {
        let output_dir = temp_dir.path();
