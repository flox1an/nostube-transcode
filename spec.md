...
## Overview
A Nostr-based Decentralized Virtual Machine (DVM) that accepts video URLs, processes them into either HLS (HTTP Live Streaming) or MP4 format with configurable resolutions (480p, 720p, 1080p), and uploads the results to Blossom servers.
...
2. **Video Processing Engine**
   ...
   - Transcodes to HLS or MP4 format based on request parameters
   - Supports configurable resolutions: 480p (854x480), 720p (1280x720), 1080p (1920x1080)
   - For HLS: Generates master and stream playlists with fMP4 segments
   - For MP4: Creates standalone files with fast start enabled
   - Generates content-addressable filenames (SHA-256 hashes)
   - Uses H.265 (libx265) codec for efficient compression
...
- `output`: Output format (legacy, use param instead)
...
**Output Formats:**
1. **HLS (HTTP Live Streaming)**: Generates adaptive bitrate streaming
   - Creates master.m3u8 playlist
   - Creates stream playlists for each resolution
   - Generates fMP4 segments
   - Suitable for streaming applications

2. **MP4 (Single File)**: Generates standalone MP4 files
   - One file per requested resolution
   - H.265 codec with CRF 28
   - Fast start enabled (moov atom at beginning)
   - Suitable for downloads and simple playback
...
If no resolutions are specified, all three are generated by default.
